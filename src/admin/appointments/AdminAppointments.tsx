            ...
            >
              <div className="flex-1 space-y-4 w-full">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between md:justify-start gap-3">
                  <h3 className="text-xl font-bold text-black dark:text-white flex items-center gap-2 min-w-0">
                    <User className="w-5 h-5 shrink-0 text-cyan-500" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-8 text-sm text-zinc-600 dark:text-zinc-400 bg-zinc-50/50 dark:bg-zinc-950/30 p-3 rounded-xl border border-black/5 dark:border-white/5">
                  <div className="flex items-center gap-2 min-w-0">
                    <Calendar className="w-4 h-4 shrink-0 text-cyan-500/70" />
                    <span className="truncate font-medium">{new Date(apt.date).toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}</span>
                  </div>
                  <div className="flex items-center gap-2 min-w-0">
                    <Clock className="w-4 h-4 shrink-0 text-cyan-500/70" />
                    <span className="truncate font-medium">{apt.timeSlot}</span>
                  </div>
                  <div className="flex items-center gap-2 min-w-0 group/info">
                    <Mail className="w-4 h-4 shrink-0 text-cyan-500/70" />
                    <span className="truncate font-medium select-all" title={apt.clientEmail}>{apt.clientEmail}</span>
                  </div>
                  <div className="flex items-center gap-2 min-w-0">
                    <Phone className="w-4 h-4 shrink-0 text-cyan-500/70" />
                    <span className="truncate font-medium select-all">{apt.clientPhone}</span>
                  </div>
                </div>
              ...
              {/* Actions */}
              <div className="flex flex-col gap-2 w-full md:w-56 shrink-0 mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-black/5 dark:border-white/5">
                <div className="flex flex-row md:flex-col gap-2">
                  {apt.status === 'PENDING' && (
                    <>
                      <button onClick={() => updateStatus(apt.id, 'APPROVED')} className="flex-1 flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2.5 rounded-xl font-bold transition-all hover:shadow-lg hover:shadow-emerald-500/20 active:scale-95">
                        <CheckCircle className="w-4 h-4" /> Approve
                      </button>
                      <button onClick={() => updateStatus(apt.id, 'REJECTED')} className="flex-1 flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2.5 rounded-xl font-bold transition-all hover:shadow-lg hover:shadow-red-500/20 active:scale-95">
                        <XCircle className="w-4 h-4" /> Reject
                      </button>
                    </>
                  )}
                  {apt.status === 'APPROVED' && (
                    <button onClick={() => updateStatus(apt.id, 'CANCELLED')} className="flex-1 flex items-center justify-center gap-2 bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 px-4 py-2.5 rounded-xl font-bold transition-all active:scale-95">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <a href={getEmailLink(apt)} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 bg-blue-50 dark:bg-blue-500/10 hover:bg-blue-100 dark:hover:bg-blue-500/20 text-blue-600 dark:text-blue-400 py-2.5 rounded-xl font-bold transition-all border border-blue-100 dark:border-blue-500/20">
                    <Mail className="w-4 h-4" /> Email
                  </a>
                  <a href={getWhatsAppLink(apt)} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] py-2.5 rounded-xl font-bold transition-all border border-[#25D366]/20">
                    <MessageCircle className="w-4 h-4" /> WA
                  </a>
                </div>

                <button onClick={() => deleteAppointment(apt.id)} className="w-full flex items-center justify-center gap-2 text-zinc-400 hover:text-red-500 py-2 rounded-xl font-medium text-xs transition-colors mt-1">
